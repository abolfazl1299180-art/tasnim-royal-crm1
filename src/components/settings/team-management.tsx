...
export function TeamManagement({ currentUserId }: { currentUserId: string }) {
  ...
  async function updateProfile(id: string, patch: Partial<Pick<Profile, "role" | "is_active">>) {
    ...
    if (updateError || !data) {
      setError(updateError?.message || "تغییر کاربر انجام نشد.");
      await load();
    } else {
    ...
    <section className="mt-4 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm lg:col-span-2">
      ...
      <div className="mt-5 space-y-3">
        {profiles.map((profile) => {
          ...
          return (
            <div key={profile.id} className="grid gap-3 rounded-2xl border border-slate-100 bg-slate-50 p-4 md:grid-cols-[1fr_180px_140px] md:items-center">
              <div>
                <p className="font-black">{profile.full_name || profile.username}</p>
                <p className="mt-1 text-xs text-slate-400" dir="ltr">{profile.username}</p>
              </div>
              <label>
                <span className="mb-1 block text-xs font-bold text-slate-400">نقش</span>
                <select
              ...
              <label className="flex items-center gap-2 pt-5 text-sm font-bold">
                <input
                  type="checkbox"
                  disabled={locked || busy}
                  checked={profile.is_active}
                  onChange={(event) => void updateProfile(profile.id, { is_active: event.target.checked })}
                  className="h-4 w-4"
                />
                کاربر فعال
                {locked && <span className="text-xs font-normal text-slate-400">(حساب فعلی)</span>}
              </label>
            </div>
          );
        })}
