    ...
    <div className="min-h-screen bg-slate-50 text-slate-900">
      ...
      <aside className={`fixed inset-y-0 right-0 z-40 w-72 border-l border-slate-200 bg-white transition-transform duration-200 lg:translate-x-0 ${open ? "translate-x-0" : "translate-x-full"}`}>
        ...
        <nav className="space-y-1 p-4">
          {navigation.map((item) => (
            <a key={item.label} href={item.href} onClick={() => setOpen(false)} className="flex items-center gap-3 rounded-2xl px-4 py-3 text-sm font-bold text-slate-600 transition hover:bg-slate-50 hover:text-slate-950">
              <span className="flex h-8 w-8 items-center justify-center rounded-xl bg-slate-100 text-base">{item.icon}</span>{item.label}
            </a>
          ))}
        </nav>
      ...
      <div className="lg:pr-72">
        <header className="sticky top-0 z-20 flex h-20 items-center justify-between border-b border-slate-200 bg-white/90 px-4 backdrop-blur md:px-8">
          <div className="flex items-center gap-3">
            <button aria-label="باز کردن منو" className="flex h-11 w-11 items-center justify-center rounded-2xl border border-slate-200 text-lg lg:hidden" onClick={() => setOpen(true)}>☰</button>
            <div><p className="text-xs font-bold text-slate-400">پنل مدیریت</p><h1 className="text-lg font-black">CRM تسنیم رویال</h1></div>
          </div>
          <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-slate-950 text-sm font-black text-amber-400">ت</div>
        </header>
